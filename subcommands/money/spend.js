...
class Spend {
  async execute(interaction, client) {
    ...
    if (whatToSpend < 1) {
        await interaction.reply({ content: "You can't spend less than $1!", ephemeral: true })
        return
    }
    ...
    await interaction.reply({content: `${interaction.member.displayName} spent $${whatToSpend}`})
