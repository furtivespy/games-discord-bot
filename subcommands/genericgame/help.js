...
*   **/game pinnedstatus [mode:on/off/full]** 📌 - Sets a pinned live status message that is edited in place. Default is off. **on** also posts the full status table in chat; **full** updates the pin only (chat keeps each command's normal reply; **/game status** still posts the table).
...
*   **/lfg** 🙋 - Gather Interest: look up a game on BGG and post a “Who’s interested?” panel. People tap Very / Somewhat / Flexibly interested (in, but willing to give a seat to a new player, someone not in a game, or someone who’d rather play). The host can close interest or **Start game** (picks seats from interested people and/or anyone on the server, then opens a play thread in the configured games channel).
