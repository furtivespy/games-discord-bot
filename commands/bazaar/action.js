        ...
        if (gameData.turnClaim) {
          ...
          if (
            ...
            const objEmbed = new EmbedBuilder()
              .setColor(386945)
              .setTitle("Would you like to claim an objective?")
              .setTimestamp();
            objEmbed.addFields(
              {
                name: `Objectives`,
                value: `
:regional_indicator_a::arrow_right:  ${BazaarFormatter.objectiveFormat(
                  gameData.objectiveA[0]
                )} (${gameData.objectiveA.length})
:regional_indicator_b::arrow_right:  ${BazaarFormatter.objectiveFormat(
                  gameData.objectiveB[0]
                )} (${gameData.objectiveB.length})
:regional_indicator_c::arrow_right:  ${BazaarFormatter.objectiveFormat(
                  gameData.objectiveC[0]
                )} (${gameData.objectiveC.length})
:regional_indicator_d::arrow_right:  ${BazaarFormatter.objectiveFormat(
...
:no_entry_sign::arrow_right:  I do not want to claim an objective`,
              ...
              {
                name: `Players`,
                value: await BazaarFormatter.playerList(this.client, gameData),
              }
            );
                ...
                objmsg
                  ...
                  .then((r) =>
                    objmsg
                      .react("🇨")
                      .then((r) =>
                        objmsg.react("🇩").then((r) => objmsg.react("🚫"))
                      )
                  )
              );

            try {
              var collectedObjective = await objmsg.awaitReactions({
            ...
            if (!collectedObjective.first()) {
              await objmsg.edit(
                "No reaction after 60 seconds, Claim Attempt canceled - try again when ready"
              );
            ...
            await objmsg.edit(
              `You Picked ${collectedObjective.first().emoji.name}`
            );
            let claimedScore = Action.ClaimObjective(
              gameData,
              collectedObjective.first().emoji.name,
              buildHistory
            );
            ...
            if (claimedScore > 0) {
              await objmsg.edit(
                `You claimed the Objective and scored ${claimedScore} points`
              );
              this.client.setGameData(`bazaar-${message.channel.id}`, gameData);
            } else {
        ...
        if (!collected.first()) {
          await msg.edit(
            "No reaction after 60 seconds, turn attempt canceled - try again when ready"
          );
        ...
        if (collected.first().emoji.name == "🎲") {
          ...
          if (dieroll == "🌈") {
            ...
            const bazaarEmbed2 = new EmbedBuilder()
              .setColor(386945)
              .setTitle("You Rolled a Wild!")
              .setTimestamp()
              .addFields({
                name: "Pick a color",
                value: "Choose which color you'd like",
              });
            await msg.suppressEmbeds(true);
            await msg.reactions.removeAll();
              ...
              .then((r) =>
                msg
                  ...
                  .then((r) =>
                    msg
                      .react("🔴")
                      .then((r) => msg.react("⚪").then((r) => msg.react("🟡")))
                  )
              );

            try {
              var collected2 = await msg.awaitReactions({
                filter: (reaction, user) =>
                  user.id == currentPlayer &&
                  (reaction.emoji.name == "🔵" ||
                    reaction.emoji.name == "🟢" ||
        ...
        if (collected.first().emoji.name == "🛍") {
          ...
          const exchangeEmbed = new EmbedBuilder()
            .setColor(386945)
            .setTitle("Which Swap do you want to make")
            .setTimestamp()
            .addFields({
              name: `Exchanges`,
              value: `${BazaarFormatter.bazaarFormat([
          ...
          try {
            var collectedSwap = await msg.awaitReactions({
              filter: (reaction, user) =>
                user.id == currentPlayer &&
          ...
          let selectedBazaar = Action.GetSelectedBazaar(
            gameData,
            collectedSwap.first().emoji.name
          );
          const exchangeEmbed2 = new EmbedBuilder()
            .setColor(386945)
            .setTitle("Which direction do you want to swap")
            .setTimestamp()
            .addFields({
              name: `Exchanges`,
              value: `${BazaarFormatter.bazaarLeftRightFormat(selectedBazaar)}`,
            });
          await msg.suppressEmbeds(true);
          await msg.reactions.removeAll();
          ...
          msg.react("📤").then((r) => msg.react("📥"));
          try {
            var collectedDirection = await msg.awaitReactions({
              filter: (reaction, user) =>
                user.id == currentPlayer &&
                (reaction.emoji.name == "📤" || reaction.emoji.name == "📥"),
              max: 1,
          ...
          if (
            Action.ClaimExchange(
              gameData,
              selectedBazaar,
              collectedDirection.first().emoji.name,
              buildHistory
            ) < 0
          ) {
  ...
  static ClaimObjective(gameData, choice, history) {
    ...
    let currentHand = [
      ...gameData.players.find((p) => p.order == gameData.turn).hand,
    ];
    ...
    //Score
    let remaining = gameData.players.find((p) => p.order == gameData.turn).hand
      .length;
  ...
  static ClaimExchange(gameData, choice, direction, history) {
    ...
    switch (direction) {
      case "📤":
        ...
        history.exchange = BazaarFormatter.bazaarFormatDirection(
          "right",
          choice
        );
        gameData.players.find((p) => p.order == gameData.turn).hand = [
  ...
  static ClaimDieToken(gameData, dieroll) {
    switch (dieroll) {
      case "🔵":
        gameData.players
          .find((p) => p.order == gameData.turn)
          .hand.push(bazaarData.colors.BLUE);
        break;
      case "🟢":
        gameData.players
          .find((p) => p.order == gameData.turn)
          .hand.push(bazaarData.colors.GREEN);
        break;
      case "🔴":
        gameData.players
          .find((p) => p.order == gameData.turn)
          .hand.push(bazaarData.colors.RED);
        break;
      case "⚪":
        gameData.players
          .find((p) => p.order == gameData.turn)
          .hand.push(bazaarData.colors.WHITE);
        break;
      case "🟡":
        gameData.players
          .find((p) => p.order == gameData.turn)
          .hand.push(bazaarData.colors.YELLOW);
        break;
    }
    Action.NextTurn(gameData);
  }
