  ...
  async run(message, args, level) {
    try {
      let gameData = Object.assign(
        {},
        _.cloneDeep(CardDB.defaultGameData),
        this.client.getGameData(`cards-${message.channel.id}`)
      );
      if (!args[0] || !args[1]) {
        const cardsEmbed = new EmbedBuilder()
          .setColor(13928716)
          .setTitle("Add Deck for Use in Channel")
          .setTimestamp();
        let deckList = "";
        ...
        cardsEmbed.addFields(
          { name: `Avaialble Decks`, value: deckList },

          {
            name: `Instructions`,
            value: `To add a deck use command in form of &cards new local-name deck_name
        ...
        if (CardDB[args[0]]) {
          if (_.find(gameData.decks, { name: newName })) {
            await message.reply(
              `A deck named ${newName} already exists in this channel`
            );
          } else {
            gameData.decks.push({
              name: newName,
              allCards: [...CardDB[args[0]]],
              currentDeck: [...CardDB[args[0]]],
              discard: [],
            });

            this.client.setGameData(`cards-${message.channel.id}`, gameData);
            await message.reply(
              `added ${newName} to this channels cards. Remember to shuffle!`
            );
          }
        } else {
