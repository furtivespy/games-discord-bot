...
class Draw extends Command {
    ...
    try {
      ...
      if (!args[0]) {
        if (gameData.decks.length == 1) {
          ...
          await message.reply(
            "please include the name of the deck to draw from"
          );
      ...
      if (deck) {
        ...
        if (drawnCards.length > 0) {
          message.reply(
            `you drew ${drawnCards.length} ${
              drawnCards.length > 1 ? "cards" : "card"
            }`
          );
          const cardEmbed = new EmbedBuilder()
            .setColor(13928716)
            .setTitle(`You Drew:`)
            .setDescription(drawnCards.join("\n"));
  ...
  static AddHand(player, deck) {
    if (_.find(player.hands, { deck: deck.name })) {
      ...
      player.hands.push(
        Object.assign({}, _.cloneDeep(CardDB.defaultHand), { deck: deck.name })
      );
    }
  }
