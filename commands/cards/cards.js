      ...
      if (args[0]) {
        ...
        if (gameData.decks && gameData.decks.length > 0) {
          const cardsEmbed = new EmbedBuilder()
            .setColor(13928716)
            .setTitle("Current Decks In This Channel")
            .setTimestamp();
          for (let i = 0; i < gameData.decks.length; i++) {
            cardsEmbed.addFields({
              name: gameData.decks[i].name,
              value: `${gameData.decks[i].currentDeck.length} cards remaining (${gameData.decks[i].discard.length} discards)`,
            });
          }
