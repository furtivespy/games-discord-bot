...
:regional_indicator_d::arrow_right:  ${this.objectiveFormat(gameData.objectiveD[0])} (${gameData.objectiveD.length})`)
        ...
        statusEmbed.addField(`Players`, await this.playerList(client, gameData))
        var activePlayer = await guild.members.fetch(gameData.players.find(p => p.order == gameData.turn).userId)
        if (gameData.turnClaim){
            statusEmbed.addField(`Turn`, `It is still ${activePlayer.displayName}'s turn - use "&bazaar action" (or "&b a" or "&ba") to play`)
        } else {
            statusEmbed.addField(`Turn`, `It is ${activePlayer.displayName}'s turn - use "&bazaar action" (or shortcuts "&b a" or "&ba") to play`)
        }
        return statusEmbed
    }
    ...
    static previousMoveFormat(history){
       ...
       let results = ""
        ...
        if (history.objectiveClaim) {
            results += `\nThey also claimed objective ${this.objectiveFormat(history.objective)} for ${history.points} points`
        }
        return results
    }
