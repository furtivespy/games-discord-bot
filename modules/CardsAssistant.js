...
class CardsAssistant {
  static async ChooseDeck(player, message, actionName) {
    ...
    const handEmbed = new EmbedBuilder()
      .setColor(13928716)
      .setTitle(`Select a Deck To ${actionName} From`);
    let decks = "";
    for (let i = 0; i < player.hands.length && i < 10; i++) {
      decks += `${Emoji.IndexToEmoji(i)}:arrow_right:  ${
        player.hands[i].deck
      }\n`;
    }
    ...
    if (!deckReaction.first()) {
      await msg.edit(
        `No reaction after 60 seconds, ${actionName} canceled - try again when ready`
      );
  ...
  static async ChooseCard(player, hand, message, actionName, page = 0) {
    const handEmbed = new EmbedBuilder()
      .setColor(13928716)
      .setTitle(`Select a Card To ${actionName} from ${hand.deck}`);
    let cards = "";
    if (page > 0) {
      cards += `⏮️:arrow_right: (Previous Page)\n`;
    }
    for (let i = 0; i + page * 10 < hand.cards.length && i < 10; i++) {
      cards += `${Emoji.IndexToEmoji(i)}:arrow_right:  ${
        hand.cards[i + page * 10]
      }\n`;
    }
    ...
    if (deckReaction.first().emoji.name == "⏭️") {
      return await CardsAssistant.ChooseCard(
        player,
        hand,
        message,
        actionName,
    ...
    return hand.cards[
      Emoji.EmojiToIndex(deckReaction.first().emoji.name) + page * 10
    ];
  }
