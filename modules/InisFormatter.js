...
const fuseSearch = new Fuse(
  [...allCards.seasons, ...allCards.epics, ...allCards.lands],
  { keys: ["name"] }
);

class InisFormatter {
  static async gameStatus(client, gameData) {
    var guild = await client.guilds.fetch(
      gameData.players.find((p) => p.isBrenn).guildId
    );
    ...
    await Promise.all(
      _.orderBy(gameData.players, "order").map(async (player) => {
        ...
        statusEmbed.addFields(
          {name: `${player.isBrenn ? ":crown: " : ""}${user.displayName}`,
          value: `${player.hand.length} card${player.hand.length == 1 ? "" : "s"}`}
        );
      })
    );
    statusEmbed.addFields(
