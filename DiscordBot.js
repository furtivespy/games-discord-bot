...
class DiscordBot extends Client {
  ...
  TryExecuteCommand(command, message, args) {
    const cmd = this.commands.get(command) || this.commands.get(this.aliases.get(command));
    if (!cmd) return;
    if (!cmd.conf.enabled) return

    const exclusions = this.getExclusions(message.guild)
    if(exclusions.includes(cmd.help.name)) return
    
    if (cmd && !message.guild && cmd.conf.guildOnly)
      return message.channel.send("This command is unavailable via private message. Please run this command in a guild.");
    
    const level = this.permlevel(message);
    if (level < this.levelCache[cmd.conf.permLevel]) {
      if (settings.systemNotice === "true") {
        return message.channel.send(`You do not have permission to use this command.
        Your permission level is ${level} (${this.permLevels.find(l => l.level === level).name})
        This command requires level ${this.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})`);
      } else {
        return;
      }
    }
...
client.on('raw', async event => {
  ...
  client.logger.log(reaction, "debug")
